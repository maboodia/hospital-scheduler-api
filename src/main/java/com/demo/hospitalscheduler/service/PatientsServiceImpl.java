...
public class PatientsServiceImpl implements PatientsService {
    ...
    @Autowired
    public PatientsServiceImpl(DoctorRepository doctorRepository, PatientRepository patientRepository, ScheduleRepository scheduleRepository) {
    ...
    */
    ...
    /*
        Remove a schedule
     */
    public ResponseEntity removeSchedule(Long patientId, Long scheduleId) {

        if(!this.patientRepository.existsById(patientId)) {
            return new ResponseEntity<>("No Patient Found for Id: " + patientId, HttpStatus.NOT_FOUND);
        }

        if(!this.scheduleRepository.findByIdAndPatientId(scheduleId, patientId).isEmpty()) {
